            ...
            {result.platesNeeded.length > 0 ? (
              <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-slate-200">
                  <div className="text-sm text-slate-600 mb-1">Total loaded:</div>
...
function WorkingWeightCalculator({ weightUnit = 'lbs' }) {
  ...
  return (
    <div className="space-y-5">
      {/* Known performance */}
      <div>
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">What you know</p>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            <Label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Reps done</Label>
            <Input type="number" min="1" max="36" placeholder="5"
              value={knownReps} onChange={(e) => setKnownReps(e.target.value)} />
          </div>
          <div>
            <Label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">RIR left</Label>
      ...
      {/* Target */}
      <div>
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">What you want to do</p>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Target reps</Label>
            <Input type="number" min="1" max="36" placeholder="3"
              value={targetReps} onChange={(e) => setTargetReps(e.target.value)} />
          </div>
          <div>
            <Label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Target RIR</Label>
            <Input type="number" min="0" max="10" placeholder="2"
        ...
        <Card className="bg-primary-50 border-primary-200">
          <CardContent className="pt-6 space-y-4">
            <div className="text-center">
              <div className="text-sm text-slate-500 mb-1">Working Weight</div>
            ...
            <div className="grid grid-cols-3 gap-3 pt-3 border-t border-primary-200">
              <div className="text-center">
                <div className="text-xs text-slate-500 mb-0.5">Est. 1RM</div>
                <div className="font-semibold text-slate-800">{result.estimatedOneRM.toFixed(1)} {weightUnit}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500 mb-0.5">% of 1RM</div>
                <div className="font-semibold text-slate-800">{result.targetPct.toFixed(1)}%</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500 mb-0.5">Exact weight</div>
                <div className="font-semibold text-slate-800">{result.exact.toFixed(1)} {weightUnit}</div>
              </div>
            </div>
          </CardContent>
        </Card>
