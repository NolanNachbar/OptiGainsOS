...
export default function MealTemplates({ compact = false }) {
  ...
  return (
    <div className="space-y-4">
      {!compact && (
        <div>
          <h2 className="text-xl font-bold text-left text-slate-900 dark:text-white">Meal Templates</h2>
      ...
      {sortedTemplates.length === 0 ? (
        ...
        <div className="space-y-2">
          {sortedTemplates.map((template) => {
            ...
            return (
              <div key={template.id} className="p-3 rounded-xl border border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-800/60 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <span className="text-sm font-semibold text-slate-900 dark:text-white truncate block">{template.name}</span>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {template.is_favorite && <Star className="w-3 h-3 text-yellow-500 fill-yellow-500 shrink-0" />}
                      <span className="text-xs text-slate-500 dark:text-slate-500 capitalize">{template.template_type === "day" ? "Full Day" : template.meal_type || "Meal"}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      onClick={() => toggleFavoriteMutation.mutate({ id: template.id, is_favorite: !template.is_favorite })}
                      className={`p-1.5 transition-colors ${template.is_favorite ? 'text-yellow-500 hover:text-yellow-400' : 'text-slate-300 dark:text-slate-600 hover:text-yellow-500'}`}
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors"
                    >
                ...
                <div className="flex gap-3 text-[11px] font-mono">
                  <span className="text-slate-900 dark:text-white font-bold">{Math.round(totals.calories)}<span className="text-slate-400 dark:text-slate-500 font-normal ml-0.5">cal</span></span>
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg flex flex-col p-0 overflow-hidden max-h-[90vh]">
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-6 py-4 space-y-3" style={{ WebkitOverflowScrolling: 'touch' }}>
            ...
            <div className="relative">
              ...
              {searchResults.length > 0 && (
                <div className="absolute z-10 top-full mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-52 overflow-y-auto">
                  {searchResults.map((food) => (
                    <button
                      key={food.fdcId}
                      onClick={() => addFromSearch(food)}
                      className="w-full text-left px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors border-b border-slate-100 dark:border-slate-700/60 last:border-0"
                    >
                      <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{food.description}</p>
            ...
            <div className="space-y-2">
              {items.length === 0 && (
                <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-6">No items yet. Search above or add manually.</p>
              )}
              {items.map((item, idx) => (
                <div key={idx} className="rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
                  {/* Row header — click to expand */}
                  <div
                    className="flex items-center gap-2 px-3 py-2.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
                    onClick={() => setExpandedIndex(expandedIndex === idx ? null : idx)}
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{item.food_name || <span className="text-slate-400 italic">Unnamed</span>}</p>
                      <p className="text-xs font-mono text-slate-500 dark:text-slate-400">{item.calories} cal · P{item.protein_grams}g · C{item.carbs_grams}g · F{item.fats_grams}g</p>
                    </div>
                  ...
                  {/* Expanded edit form */}
                  {expandedIndex === idx && (
                    <div className="px-3 pb-3 pt-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 space-y-2">
                      <div>
            ...
            <button
              onClick={addManual}
              className="w-full flex items-center justify-center gap-1.5 py-2 text-xs font-semibold text-slate-400 hover:text-violet-600 dark:hover:text-violet-400 border border-dashed border-slate-200 dark:border-slate-700 rounded-lg hover:border-violet-400 transition-colors"
            >
          ...
          {/* Footer */}
          <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shrink-0 space-y-2">
            <Button onClick={handleSave} disabled={isSaving} className="w-full bg-primary-600 hover:bg-primary-700">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {entries.map((e, idx) => (
              <div key={idx} className="flex justify-between p-2 bg-slate-50 rounded text-sm">
                <span className="text-slate-900">{e.food_name}</span>
