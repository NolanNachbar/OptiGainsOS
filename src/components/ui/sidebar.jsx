...
export const ActionBar = ({ className = "", children, ...props }) => {
  ...
  return (
    <>
      ...
      {/* Header Navigation */}
      <nav
        className={`fixed md:sticky top-0 left-0 right-0 w-full h-screen md:h-auto bg-white dark:bg-slate-900 flex flex-col md:flex-row md:items-center md:px-6 md:py-4 flex-shrink-0 transition-transform duration-300 z-50 ${
          open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        } md:border-b md:border-slate-200 dark:md:border-slate-700 ${className}`}
        {...props}
      >
        {children}
      </nav>
