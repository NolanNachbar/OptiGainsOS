...
export function WeightProgressChart({ data, weightUnit = 'lbs' }) {
  if (!data || data.length === 0) {
    return (
      <div className="w-full h-80 flex items-center justify-center bg-slate-50 rounded-lg dark:bg-slate-800 dark:text-white">
    ...
    <div className="w-full">
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-slate-50 rounded-lg p-4">
          ...
          <div className="text-xl font-bold text-slate-900">{startWeight} {weightUnit}</div>
        </div>
        <div className="bg-slate-50 rounded-lg p-4">
          <div className="text-xs text-slate-500 mb-1">Current</div>
          <div className="text-xl font-bold text-slate-900">{currentWeight} {weightUnit}</div>
        </div>
        <div className="bg-slate-50 rounded-lg p-4">
        ...
        <div className="bg-slate-50 rounded-lg p-4">
          ...
          <div className={`text-xl font-bold ${weightChange > 0 ? 'text-warning-600' : weightChange < 0 ? 'text-success-600' : 'text-slate-900'}`}>
