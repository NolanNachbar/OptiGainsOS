...
function PSTBar({ event, value }) {
  ...
  if (!value) return (
    <div className="flex items-center justify-between text-sm mb-3">
      <span className="text-[#a0a0a0] w-28">{cfg.label}</span>
      <span className="text-[#555555] text-xs">Not logged</span>
    </div>
  );
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between text-xs mb-1">
        <span className="text-[#a0a0a0]">{cfg.label}</span>
        <span className={`font-bold ${isGood ? "text-brand" : "text-[#a0a0a0]"}`}>
          {display} <span className="text-[#555555] font-normal">/ target {targetDisplay}</span>
        </span>
      </div>
  ...
  return (
    <Card className="bg-[#1a1a1a] border-[#2a2a2a]">
      <CardHeader className="pb-2 pt-4 px-5">
        ...
        {latest?.test_date && (
          <p className="text-[10px] text-[#555555] mt-1">Last tested: {latest.test_date}</p>
        )}
      </CardHeader>
      <CardContent className="px-5 pb-4">
        {isLoading ? (
          ...
          <div className="text-center py-4">
            <Trophy className="w-8 h-8 mx-auto text-[#333] mb-2" />
            <p className="text-sm text-[#555555]">No PST data yet.</p>
            <p className="text-xs text-[#444] mt-1">Log your first test to track progress.</p>
          </div>
        ) : (
          <>
            ...
            {prev && (
              <div className="mt-3 pt-3 border-t border-[#2a2a2a]">
                <p className="text-[10px] text-[#555555] uppercase tracking-wider mb-2">vs previous ({prev.test_date})</p>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="text-xs text-[#a0a0a0] mb-1 block">500yd Swim (min:sec)</label>
            ...
            <div>
              <label className="text-xs text-[#a0a0a0] mb-1 block">1.5mi Run (min:sec)</label>
              <div className="flex gap-2">
            ...
            <div>
              <label className="text-xs text-[#a0a0a0] mb-1 block">Notes</label>
