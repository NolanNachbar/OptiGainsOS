...
export default function PublicProfile() {
    ...
    return (
      <div className="bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-300">
        ...
        <div className="p-4 md:p-6">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white mb-6">
          ...
          <div className="max-w-5xl mx-auto">
            <Card className="border-none shadow-lg">
              <CardContent className="py-12 text-center">
                ...
                <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">User not found</h3>
                <p className="text-slate-500">No user with the username "@{username}" exists.</p>
              </CardContent>
            </Card>
          </div>
        </div>
          ...
          <div className="max-w-5xl mx-auto">
            <Card className="border-none shadow-lg">
              <CardContent className="py-12 text-center">
                ...
                <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">@{profile.username}</h3>
  ...
  const ldJson = {
    ...
    "description": profile.bio || `Fitness profile for ${profile.username} on Sisyphus' Schedule`,
    "image": profile.avatar_url,
    "url": `https://sisyphusschedule.com/profile/${profile.username}`,
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://sisyphusschedule.com/profile/${profile.username}`
    }
  };
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-300">
      <SEO 
        ...
        description={profile.bio || `Check out ${profile.display_name || profile.username}'s fitness progress, shared workouts, and training programs on Sisyphus' Schedule.`}
          ...
          <CardContent className="pt-8 pb-6">
            <div className="flex flex-col items-center text-center">
              ...
              {profile.display_name && (
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{profile.display_name}</h2>
              )}
              <p className={`${profile.display_name ? 'text-sm text-slate-500' : 'text-2xl font-bold text-slate-900 dark:text-white'}`}>@{profile.username}</p>
              {profile.bio && (
                <p className="text-slate-600 dark:text-slate-400 mt-2 max-w-md">{profile.bio}</p>
              )}
              {profile.isOwn && (
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <Card className="border-none shadow-sm">
            <CardContent className="py-4 text-center">
              ...
              <p className="text-2xl font-bold text-slate-900 dark:text-white">{profile.total_workouts || 0}</p>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="workouts">
                {loadingWorkouts ? (
                  ...
                  <p className="text-center text-slate-500 py-6">No shared workouts yet</p>
                ) : (
                  <div className="space-y-4">
                    {sharedWorkouts.map((sw) => (
                      <Card key={sw.id} className="bg-slate-50">
                        <CardContent className="pt-5 pb-4">
                          <div className="flex gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between mb-3">
                                <div>
                                  <h4 className="font-semibold text-slate-900 dark:text-white">{sw.workout_title}</h4>
                                  {sw.caption && (
                                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{sw.caption}</p>
                                  )}
                                ...
                                {!profile.isOwn && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="shrink-0 text-primary-600 border-primary-200 hover:bg-primary-50"
              ...
              <TabsContent value="programs">
                {loadingPrograms ? (
                  ...
                  <p className="text-center text-slate-500 py-6">No shared programs yet</p>
                ) : (
                  <div className="space-y-4">
                    {sharedPrograms.map((sp) => {
                      ...
                      return (
                        <Card key={sp.id} className="bg-slate-50">
                          <CardContent className="pt-5 pb-4">
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex-1">
                                <div className="flex items-start gap-2 mb-2">
                                  ...
                                  <div>
                                    <h4 className="font-semibold text-slate-900 dark:text-white">{program?.name || 'Untitled Program'}</h4>
                                    {sp.caption && (
                                      <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{sp.caption}</p>
                                    )}
                                  </div>
                                </div>
                                  ...
                                  <div className="space-y-2 mt-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                      <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-slate-600">{program.cycle_length}-day cycle</span>
                                      </div>
                                      <div className="flex items-center gap-1.5">
                                        <Repeat className="w-3.5 h-3.5 text-slate-400" />
                                        <span className="text-slate-600">{program.num_cycles} cycles</span>
                                      </div>
                                    </div>
