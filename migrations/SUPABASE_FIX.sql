-- SUPABASE SCHEMA FIX: Run this in your Supabase SQL Editor.
-- This script fixes 404 (Not Found) and 400 (Bad Request) errors.

-- 1. Standardize existing columns from 'user_id' to 'created_by'
DO $$ 
BEGIN
...
-- 2. Create missing tables with 'created_by'

-- Recovery Metrics (Garmin/Apple Health data)
...
-- Capture Inbox (Second Brain Notes Stream)
...
-- Exercise Reactions (Fixes 404 in Workouts tab)
...
-- 3. Security: Enable RLS and add All-Access Policies for Personal OS
...
DO $$ 
BEGIN
    DROP POLICY IF EXISTS "Personal OS all access recovery" ON recovery_metrics;
    CREATE POLICY "Personal OS all access recovery" ON recovery_metrics FOR ALL USING (auth.uid() = created_by);

    DROP POLICY IF EXISTS "Personal OS all access capture" ON capture_inbox;
    CREATE POLICY "Personal OS all access capture" ON capture_inbox FOR ALL USING (auth.uid() = created_by);

    DROP POLICY IF EXISTS "Personal OS all access reactions" ON exercise_reactions;
    CREATE POLICY "Personal OS all access reactions" ON exercise_reactions FOR ALL USING (auth.uid() = created_by);

    DROP POLICY IF EXISTS "Personal OS all access briefs" ON daily_briefs;
    CREATE POLICY "Personal OS all access briefs" ON daily_briefs FOR ALL USING (auth.uid() = created_by);
END $$;
